...
class MusicXmlViewerWidget extends HookWidget {
    ...
    controller
      ...
      ..addJavaScriptChannel(
        ...
        onMessageReceived: (JavaScriptMessage message) {
          final transposeValue = int.tryParse(message.message);
          if (transposeValue != null) {
            currentTranspose.value = transposeValue;
            playlistBloc.add(
              UpdateMusicSheetTranspositionEvent(
                musicSheet: musicSheet,
                transposition: transposeValue,
              ),
            );
          }
        },
  ...
  Future<void> _initialize(
    ...
    try {
      if (context.mounted) {
        controller.value = await _initializeWebView(context, currentTranspose);
      }
    } catch (e, st) {
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      ...
      _initialize(context, controller, currentTranspose);
      return null;
    }, const []);
